...
class music(commands.Cog):
    ...
    @commands.command(name="stop", help="Para de tocar as músicas", aliases=['parar'])
    @commands.has_permissions(manage_channels=True)
    async def stop(self, ctx):
      self.vc.stop()
      await self.vc.disconnect()

    @commands.command(name="pause", help="Pausa a música",aliases=['pausar'])
    async def pause(self, ctx):
      if self.vc.is_playing():
        ...
        embedvc = discord.Embed(
          colour=1646116,
          description=f"Não há música tocando no momento"
        )
        await ctx.send(embed=embedvc)

    @commands.command(name="resume", help="Resume a música", aliases=['resumir'])
    async def resume(self,ctx):
      if self.vc.is_paused():
        self.vc.resume()
          ...
          embedvc = discord.Embed(
          colour=1646116,
          description=f"O áudio não está pausado"
        )
          await ctx.send(embed=embedvc)
